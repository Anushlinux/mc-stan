...
export class MissionControlStore {
  ...
  syncAgents(agents: Iterable<AgentState>): void {
    let changed = false;
    const presentAgentIds = new Set<number>();
    ...
    for (const [agentIdKey, sessionId] of Object.entries(this.state.activeSessionByAgentId)) {
      const agentId = Number(agentIdKey);
      if (presentAgentIds.has(agentId)) continue;

      const session = this.state.sessions.find((candidate) => candidate.id === sessionId);
      if (!session || session.endedAt) {
        delete this.state.activeSessionByAgentId[agentId];
        changed = true;
        continue;
      }

      session.status = 'stopped';
      session.blockerReason = 'Extension session disconnected';
      session.currentTool = undefined;
      session.lastActionSummary = 'Extension session disconnected';
  ...
  recordAgentLaunchFailure(agent: AgentState, reason: string): void {
    const session = this.ensureSession(agent);
    ...
    session.endedAt = failedAt;
    this.markTaskForSession(session, 'failed', 'unknown', reason);
    this.appendEvent(session.id, 'agent_launch_failed', 'system', reason);
    this.emitChange();
  }
