...
import type { EmbeddedTerminalSnapshot } from '../../../shared/embeddedTerminal.ts';
...
export function useExtensionMessages(
  ...
  const [embeddedTerminals, setEmbeddedTerminals] = useState<Record<number, EmbeddedTerminalState>>(
    {},
  );
  ...
  useEffect(() => {
    ...
    const handler = (e: MessageEvent) => {
      ...
      } else if (msg.type === 'terminalSnapshot') {
        const agentId = msg.agentId as number;
        const snapshot = msg.snapshot as EmbeddedTerminalState;
        setEmbeddedTerminals((prev) => ({
          ...prev,
          [agentId]: snapshot,
        }));
