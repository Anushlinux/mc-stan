...
import { CanvasAgentTerminalDock } from './components/CanvasAgentTerminalDock.js';
...
function App() {
  ...
  const [canvasTerminalAgentId, setCanvasTerminalAgentId] = useState<number | null>(null);
  const [isCanvasTerminalOpen, setIsCanvasTerminalOpen] = useState(false);
  ...
  useEffect(() => {
    if (selectedAgent !== null) {
      ...
      setCanvasTerminalAgentId(selectedAgent);
      setIsCanvasTerminalOpen(true);
    }
  }, [selectedAgent]);

  useEffect(() => {
    if (canvasTerminalAgentId === null) return;
    if (agents.includes(canvasTerminalAgentId)) return;
    setCanvasTerminalAgentId(null);
    setIsCanvasTerminalOpen(false);
  }, [agents, canvasTerminalAgentId]);
  ...
  const handleClick = useCallback((agentId: number) => {
    ...
    setCanvasTerminalAgentId(inspectId);
    setIsCanvasTerminalOpen(true);
  }, []);

  const handleOpenMissionControlForAgent = useCallback((agentId: number) => {
      ...
      {!isDebugMode ? (
        <>
          ...
          {!isMissionControlOpen ? (
            <CanvasAgentTerminalDock
              agentId={canvasTerminalAgentId}
              missionControl={missionControl}
              terminal={
                canvasTerminalAgentId !== null
                  ? embeddedTerminals[canvasTerminalAgentId]
                  : undefined
              }
              isOpen={isCanvasTerminalOpen}
              onOpen={() => setIsCanvasTerminalOpen(true)}
              onClose={() => setIsCanvasTerminalOpen(false)}
              onOpenMissionControl={handleOpenMissionControlForAgent}
            />
          ) : null}
