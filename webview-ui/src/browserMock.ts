...
export function dispatchMockMessages(): void {
  ...
  dispatch({
    ...
    lastSeenVersion: '1.1',
    projectDirectories: ['/workspace/side-project'],
  });
  dispatch({
    ...
    snapshot: {
      ...
      tasks: [
        {
          ...
          ownedPaths: ['webview-ui/src/components/MissionControlModal.tsx'],
      ...
      orchestrator: {
        ...
        lastPrompt: 'Split the Mission Control build into three non-overlapping tracks.',
        lastPlanSummary:
          'Worker 1 owns the modal shell, worker 2 owns the agent inspector surfaces, and worker 3 owns the canvas terminal/dispatch fit-and-finish pass.',
        currentPhaseLabel: 'Workers Running',
        currentPhaseDetail: 'All 3 workers are active in isolated worktrees.',
        sharedConstraints: ['Keep the office canvas ambient', 'Do not rewrite the layout editor'],
        assignments: [
          {
            slot: 1,
            title: 'Mission Control shell',
            goal: 'Build the master modal shell and overview framing.',
            ownedPaths: ['webview-ui/src/components/MissionControlModal.tsx'],
            acceptanceCriteria: ['Modal switches between all primary views cleanly'],
            coordinationNotes: ['Do not edit the inspector drawer or terminal pane'],
            dependsOnSlots: [],
            agentId: 1,
            taskId: 'task-1',
            workspaceAssignmentId: 'workspace-1',
          ...
          {
            slot: 2,
            title: 'Inspector detail pass',
            goal: 'Deepen the inspector drawer and artifact detail handling.',
            ownedPaths: ['webview-ui/src/components/AgentInspectorDrawer.tsx'],
            acceptanceCriteria: ['Inspector shows richer run context'],
            coordinationNotes: ['Coordinate copy with worker 1'],
            dependsOnSlots: [1],
          },
          {
            slot: 3,
            title: 'Canvas terminal polish',
            goal: 'Tighten the canvas terminal dock and dispatch flow.',
            ownedPaths: ['webview-ui/src/components/CanvasAgentTerminalDock.tsx'],
            acceptanceCriteria: ['Terminal dock keeps session context visible'],
            coordinationNotes: ['Leave modal structure untouched'],
            dependsOnSlots: [1],
          },
        ],
        progressEntries: [
          {
            ...
            level: 'info',
            message: 'Master orchestration started. Preparing repository context.',
          },
          {
            id: 'progress-2',
            timestamp: new Date(Date.now() - 1000 * 60 * 3).toISOString(),
            level: 'success',
            message: 'Plan validated. Provisioning isolated worker worktrees.',
          },
          {
            id: 'progress-3',
            timestamp: new Date(Date.now() - 1000 * 60 * 2).toISOString(),
            level: 'info',
            message: 'Workspaces ready. Launching worker sessions.',
          },
          {
            id: 'progress-4',
            timestamp: new Date(Date.now() - 1000 * 45).toISOString(),
            level: 'success',
            message: 'Worker 1 launched successfully.',
          },
        ],
        updatedAt: new Date().toISOString(),
      },
